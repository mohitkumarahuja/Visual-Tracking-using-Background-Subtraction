...
for m = 1:size(ImSeq,3)
    ...
    subplot(223),imshow(Foreground_Threshold,[]), title('Object Detection with Threshold');
...
for m = 1: size(ImSeq,3)
    ...
    Image = imbinarize(Final);  % Converting into Black and White
...
for i = 1:size(ImSeq,3)
    ...
    Final_Image = imbinarize(Image_Reshaped);
    ...
    %Show result
    imshow(ImSeq(:,:,i),[]), title('Moving object with bounding box)');
    if(Area_Box)
        hold on;
